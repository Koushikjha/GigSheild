...
public class PaymentService {
    ...
    public void initiateClaimPayout(String claimId, Long userId, int amountInr) {
        ...
        publishCompletedAfterCommit(claimId, userId, amountInr, PaymentStatus.SUCCESS.name());
    }

    private void publishCompletedAfterCommit(String claimId, Long userId, int amountInr, String status) {
        if (TransactionSynchronizationManager.isSynchronizationActive()) {
            TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
                @Override
                public void afterCommit() {
                    payoutEventProducer.publishCompleted(claimId, userId, amountInr, status);
                }
            });
        } else {
            payoutEventProducer.publishCompleted(claimId, userId, amountInr, status);
