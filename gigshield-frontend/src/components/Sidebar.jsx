    ...
    <aside className={`sidebar ${open ? 'sidebar--open' : ''}`} aria-hidden={!open}>
      ...
      <div className="sidebar__profile">
        <div>
          ...
          <div className="sidebar__profile-name">{worker?.fullName || 'Loading…'}</div>
        </div>
        <div className="sidebar__profile-meta">
          <div>
            City: <strong>{worker?.city || '—'}</strong>
          </div>
          <div>
            Risk band: <strong>{worker?.riskBand || '—'}</strong>
          </div>
          <div>
            Fraud strikes: <strong>{worker?.fraudStrikes ?? 0}</strong>
          </div>
        </div>
      </div>
