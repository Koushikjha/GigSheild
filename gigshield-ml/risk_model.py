...
def train_and_evaluate(
    ...
    max_depth/min_samples_leaf are bounded deliberately: the training target
    is a deterministic weighted formula of the input features (see
    build_risk_target), so an *unbounded* forest over ~300k rows doesn't
    generalize better than a shallow one — it just memorizes, producing
    individual trees with hundreds of thousands of nodes. Trained without
    these bounds this model serialized to over 4 GB and OOM-killed the
    serving process on load. Bounded, it fits the same near-deterministic
    target (R² > 0.999 either way) in a few MB.
    """
