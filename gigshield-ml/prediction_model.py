...
def train_and_evaluate(X: pd.DataFrame, y: pd.Series) -> tuple[RandomForestClassifier, MinMaxScaler]:
    ...
    model = RandomForestClassifier(
        ...
        n_estimators=200,
        # Bounded for the same reason as risk_model.py's regressor: ~300k
        # training rows with no depth cap produces a needlessly huge,
        # memorized-not-generalized forest. See risk_model.py's
        # train_and_evaluate docstring for the full explanation.
        max_depth=14,
        min_samples_leaf=20,
        random_state=RANDOM_SEED,
        n_jobs=-1,
